...
function toCameraStateCamera(input: {
  id: string;
  camId: string | null;
  name: string;
}) {
...
async function loadCameraAuthorizedEmployeesState(params: {
  ...
  const employees: CameraAuthorizedStateEmployee[] = enrolledEmployees.map(
    (row) => ({
...
export async function listCameraAuthorizedEmployeePublicIds(cameraId: string) {
  ...
  return normalizeDistinctEmployeeIds(
    rows.map((row) => employeePublicId(row.employee)),
  );
}
